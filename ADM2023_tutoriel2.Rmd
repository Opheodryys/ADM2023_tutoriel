...
On retire les séquences chimères qui résultent d'un mauvais appariement lors de la PCR. Chaque séquence est passée au crible. On a ensuite le résumé.
...
On associe ensuite une appartenance taxonomique à nos séquences. Cela permettra l'interprétation de nos séquences dans l'environnement. Cette appartenance s'effectue en 2 étapes. D'abord, on utilise l'algorithme de Wang et al pour assigner la taxonomie. On utilise les séquences précedemment obtenues (sans chimères). refFasta est entraîné par silva_train_set. taxLevels attribue le niveau taxonomique grâce à une probabilité. minBoot est le niveau de confiance minimum pour l'association taxonomique (minimum 60%).
...
Puis on attribue le rang espèces aux ASV qui sont parfaitement identiques à une séquence de référence.
...
On exporte les données sous forme d'objets R (script, environnement,...), un pour la table d'ASV et une autre pour la taxonomie. On crée le fichier et on indique le chemin à R.
