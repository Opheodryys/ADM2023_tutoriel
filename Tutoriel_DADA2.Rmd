...
On associe ensuite une appartenance taxonomique à nos séquences. Cela permettra l'interprétation de nos séquences dans l'environnement. Cette appartenance s'effectue en 2 étapes. D'abord, on utilise l'algorithme de Wang et al.
...
On exporte les données sous forme d'objets R, un pour la table d'ASV et une autre pour la taxonomie.
...
On collecte les séquences ASV et on attribue des ID uniques pour chaque séquence, afin que leur nom soit plus court. On renomme les variables.
...
Ces nouvelles identités sont collectées dans une nouvelles colonne asv. 
